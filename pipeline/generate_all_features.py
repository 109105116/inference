...
def generate_rinalmo_embeddings(df: pd.DataFrame, output_dir: Path, gpu_id: int = 0, batch_size: int = 4):
    ...
    cmd = [
        "conda", "run", "-n", "rinalmo", "python", str(script_path),
        "--input", str(temp_csv),
...
def generate_secondary_structure(df: pd.DataFrame, output_dir: Path):
    ...
    cmd = [
        "conda", "run", "-n", "mxfold2", "python", str(script_path),
        "--input", str(temp_csv),
        "--output", str(ss_dir),
    ]
...
def generate_unimol_embeddings(df: pd.DataFrame, output_dir: Path, gpu_id: int = 0, batch_size: int = 32):
    ...
    cmd = [
        "conda", "run", "-n", "unimol", "python", str(script_path),
        "--input", str(temp_csv),
        "--output", str(embeddings_dir),
        "--batch-size", str(batch_size),
    ]
...
def generate_rna_onehot(df: pd.DataFrame, output_dir: Path):
...
def generate_mol_features(df: pd.DataFrame, output_dir: Path):
...
def generate_pssm(df: pd.DataFrame, output_dir: Path, database_path: str = None):
    ...
    cmd = [
        "conda", "run", "-n", "mmseqs2", "python", str(script_path),
        "--input", str(temp_csv),
    ...
    if not args.mol_only:
        if not args.skip_rinalmo:
            results['rinalmo'] = generate_rinalmo_embeddings(
                df, output_dir, args.gpu, args.rinalmo_batch_size
            )
        
        if not args.skip_mxfold2:
            results['mxfold2'] = generate_secondary_structure(df, output_dir)
        ...
        results['rna_onehot'] = generate_rna_onehot(df, output_dir)
    
    # Molecule features
    if not args.rna_only:
        if not args.skip_unimol:
            results['unimol'] = generate_unimol_embeddings(
                df, output_dir, args.gpu, args.unimol_batch_size
            )
        
        if not args.skip_mol_features:
            results['mol_features'] = generate_mol_features(df, output_dir)
