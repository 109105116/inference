...
setup_rinalmo() {
    ...
    # Use RiNALMo's official environment.yml
    if [ -f "$INFERENCE_DIR/rinalmo/environment.yml" ]; then
        echo "Creating environment from RiNALMo's environment.yml..."
        conda env create -f "$INFERENCE_DIR/rinalmo/environment.yml"
        
        # Install RiNALMo package
        echo "Installing RiNALMo package..."
        conda run -n rinalmo pip install "$INFERENCE_DIR/rinalmo"
        
        echo "RiNALMo environment setup complete!"
    else
        echo "ERROR: RiNALMo environment.yml not found at $INFERENCE_DIR/rinalmo/environment.yml"
        exit 1
    fi
}
